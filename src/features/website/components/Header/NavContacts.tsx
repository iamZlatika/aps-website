  ...
  <div className="hidden grid-cols-[auto_auto] items-center gap-x-[22px] gap-y-[5px] xl:grid">
    {contacts.map((c) => (
      <div key={c.phone} className="contents">
        <span className="flex items-center gap-2 whitespace-nowrap text-[13px] font-medium leading-[1.2] text-ws-ink-soft">
          <PinIcon className="size-3.5 shrink-0 text-ws-ember-bright opacity-90" />
          {c.address}
        </span>
        <a
          href={`tel:${c.phone}`}
          className="flex items-center gap-2 whitespace-nowrap text-[13px] font-semibold leading-[1.2] tracking-[0.005em] text-ws-ink no-underline transition-colors duration-200 hover:text-ws-ember-bright"
        >
